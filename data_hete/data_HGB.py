...
class data_loader:
    def __init__(self, path):
        ...
        #print("self.labels_train : ",self.labels_train)
        self.labels_test = self.load_labels('label.dat.test')
        print("self.labels_test:",self.labels_test)
    ...
    def load_labels(self, name):
        """
        return labels dict
            data:  (node_id, node_type_id, node_label).
            num_classes: total class number of labels
    ...
    def load_links(self):
        """
        return links dict
            data:  (node_id_source, node_id_target, edge_type_id, edge_weight),
        ...
        """
        In order to make a hypergraph form here, for each paper, -loop all other entities from this paper to the other
        """
    ...
    def load_nodes(self):
        ...
        return nodes dict
            ...
            shift: the idex that node form one type to another
                    node_id shift for each type. You can get the id range of a type by 
                        [ shift[node_type], shift[node_type]+count[node_type] )
        """
        nodes = {'total':0, 'count':Counter(), 'attr':{}, 'shift':{}}
        print("os.path.join(self.path, 'node.dat')", os.path.join(self.path, 'node.dat'))
